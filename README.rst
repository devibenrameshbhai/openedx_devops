...
- CDN at https://cdn.courses.minischool.co.in linked to a public read-only S3 bucket named courses-minischool-ap_south-storage
- public ssh access via a t2.micro Ubuntu 20.04 LTS bastion EC2 instance at bastion.courses.minischool.co.in
- daily data backups archived into a private S3 bucket named prod-minischool-ap_south-mongodb-backup
...
- CDN at https://cdn.dev.courses.minischool.co.in linked to an S3 bucket named dev-minischool-ap_south-storage
- daily data backups archived into an S3 bucket named dev-minischool-ap_south-mongodb-backup
...
.. code-block:: hcl
  ...
  locals {
    ...
    platform_region  = "ap_south"
    root_domain      = "minischool.co.in.ai"
    aws_region       = "ap-south-1"
    account_id       = "699138137328"
    ec2_ssh_key_name = "minikey"
  }
...
.. code-block:: shell
  ...
  ssh bastion.courses.minischool.co.in -i path/to/minikey.pem
...
Use `this automated Github Actions workflow <https://github.com/devibenrameshbhai/openedx_devops/actions/workflows/tutor_build_image.yml>`_ to build a customized Open edX Docker container based on the latest stable version of Open edX (current maple.2) and
...
Use `this automated Github Actions workflow <https://github.com/devibenrameshbhai/openedx_devops/actions/workflows/tutor_deploy_prod.yml>`_ to deploy your customized Docker container to a Kubernetes Cluster.
...
- **MongoDB**. uses `AWS DocumentDB <https://aws.amazon.com/documentdb/>`_ for all MongoDB data, accessible insid the vpc as mongodb.master.courses.minischool.co.in:27017 and mongodb.reader.courses.minischool.co.in. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/components/documentdb>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
